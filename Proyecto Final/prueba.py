...
# Función para validar la existencia de un producto en la base de datos
def validar_existencia_producto(id_producto):
    cursor = db.cursor()
    query = "SELECT id FROM productos WHERE id = %s"
...
def validar_existencia_cliente(id_cliente):
    cursor = db.cursor()
...
def generar_reporte_ventas_por_cliente():
    # Consultar la base de datos y obtener los datos de ventas por cliente
    cursor = db.cursor()
    selectQuery = "SELECT id_cliente, SUM(total) FROM ventas GROUP BY id_cliente"
    cursor.execute(selectQuery)
    ventas_por_cliente = cursor.fetchall()

    # Función para generar un informe de ventas por cliente en formato de documento de Word
def generar_reporte_ventas_por_cliente():
    # Consultar la base de datos y obtener los datos de ventas por cliente
    cursor = db.cursor()
    selectQuery = "SELECT id_cliente, SUM(total) FROM ventas GROUP BY id_cliente"
    ...
    document = Document()
    document.add_heading('Informe de Ventas por Cliente', 0)

    # Agregar los datos al documento
    for id_cliente, total_venta in ventas_por_cliente:
        document.add_paragraph(f'Cliente: {id_cliente}')
        document.add_paragraph(f'Total de Ventas: {total_venta}')

    # Guardar el documento en un archivo
    informe_path = 'informe_ventas_cliente.docx'
    document.save(informe_path)

    return informe_path
# Función para generar un reporte de ventas por producto en Excel
...
def generar_reporte_ventas_por_producto():
    ...
    # Query the database and get sales data by product
    selectQuery = "SELECT id_producto, SUM(cantidad) FROM ventas GROUP BY id_producto"
    cursor.execute(selectQuery)
    ventas_por_producto = cursor.fetchall()
    
    # Crear un Excel workbook
    workbook = openpyxl.Workbook()
    worksheet = workbook.active
    worksheet.title = 'Reporte de Ventas por Producto'
    
    # Agregar los datos al Excel workbook
    for producto, total_cantidad in ventas_por_producto:
        worksheet.append([producto, total_cantidad])
    
    # Guardar el Excel workbook
    reporte_path = 'reporte_ventas_producto.xlsx'
    workbook.save(reporte_path)
    
    # Cerrar el nuevo cursor
    cursor.close()
    
    # Retornar la ruta del informe generado
    return reporte_path


    # Procesar los resultados como desees
    for producto, total_cantidad in resultados_ventas_por_producto:
        print(f"Producto: {producto}, Total Cantidad: {total_cantidad}")
# Función para generar reportes de ventas por cliente y producto
def generar_reportes():
    reporte_ventas_cliente = generar_reporte_ventas_por_cliente()
    reporte_ventas_producto = generar_reporte_ventas_por_producto()

    if reporte_ventas_cliente and reporte_ventas_producto:
        enviar_reportes_por_correo(reporte_ventas_cliente, reporte_ventas_producto)
        print("Reportes generados y enviados con éxito.")
    else:
        print("Error al generar los informes.")# Función para generar reportes de ventas por cliente y producto
def generar_reportes():
    reporte_ventas_cliente = generar_reporte_ventas_por_cliente()
    reporte_ventas_producto = generar_reporte_ventas_por_producto()
...
# Función para enviar reportes por correo electrónico
def enviar_reportes_por_correo(reporte_ventas_cliente, reporte_ventas_producto, mensaje="Adjunto informes de lo obtenido", destinatario="quien recibe"):
    # Configurar el servidor SMTP
    smtp_server = "smtp.gmail.com"
    smtp_port = 587
    smtp_user = 'mi correo electronico con el que genere la contraseña'
    smtp_password = 'Aqui va la contraseña, que no puedo dar'
    ...
    # Adjunta los informes al mensaje
    with open(reporte_ventas_cliente, 'rb') as attachment:
        part = MIMEBase('application', 'octet-stream')
        part.set_payload((attachment).read())
        encoders.encode_base64(part)
        part.add_header('Content-Disposition', "attachment; filename=informe_ventas_cliente.docx")
        msg.attach(part)
    
    with open(reporte_ventas_producto, 'rb') as attachment:
        part = MIMEBase('application', 'octet-stream')
        part.set_payload((attachment).read())
        encoders.encode_base64(part)
        part.add_header('Content-Disposition', "attachment; filename=reporte_ventas_producto.xlsx")
        msg.attach(part)
...
tab_control.add(tab4, text='Reportes')
tab_control.pack(expand=1, fill="both")

# Botones y widgets para la pestaña de Reportes
generar_reportes_button = ttk.Button(tab4, text="Generar Reportes", command=generar_reportes)
enviar_reportes_button = ttk.Button(tab4, text="Enviar Reportes por Correo", command=enviar_reportes_por_correo)
generar_reportes_button.pack()
enviar_reportes_button.pack()
